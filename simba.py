...
class SimBA:
    ...
    def simba_batch(self, images_batch, labels_batch, max_iters, freq_dims, stride, epsilon, linf_bound=0.0,
        ...
        if order == 'rand':
            ...
            indices = utils_simba.diagonal_order(image_size, 3)[:max_iters]
        elif order == 'strided':
            indices = utils_simba.block_order(image_size, 3, initial_size=freq_dims, stride=stride)[:max_iters]
        else:
            indices = utils_simba.block_order(image_size, 3)[:max_iters]
        if order == 'rand':
        ...
        if pixel_attack:
            ...
            trans = lambda z: utils_simba.block_idct(z, block_size=image_size, linf_bound=linf_bound)
